...
func newMinter(config *params.ChainConfig, bxm *RaftService, blockTime time.Duration) *minter {
	minter := &minter{
		config:           config,
		bxm:              bxm,
		mux:              bxm.EventMux(),
	...
	minter.chainHeadSub = bxm.BlockChain().SubscribeChainHeadEvent(minter.chainHeadChan)
	minter.txPreSub = bxm.TxPool().SubscribeTxPreEvent(minter.txPreChan)
...
func (minter *minter) getTransactions() *types.TransactionsByPriceAndNonce {
	allAddrTxes, err := minter.bxm.TxPool().Pending()
...
func (minter *minter) mintNewBlock() {
	...
	// NOTE: < BitmedChain creates a signature here and puts it in header.Extra. >
