...
import (
	...
	"github.com/InsighterInc/bxmp/internal/jsre"
	"github.com/InsighterInc/bxmp/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute BitMED requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var bxm = web3.bxm; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = bxm.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines bxm.getBalance(bxm.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	c.jsre.Run(`
		...
		console.log("coinbase: " + bxm.coinbase);
		console.log("at block: " + bxm.blockNumber + " (" + new Date(1000 * bxm.getBlock(bxm.blockNumber).timestamp) + ")");
