...
func newTester(t *testing.T, confOverride func(*bxm.Config)) *tester {
	...
	// Create a networkless protocol stack and start an BitMED service within
	...
	bxmConf := &bxm.Config{
		...
		Bxmbase: common.HexToAddress(testAddress),
		PowTest:   true,
	}
	if confOverride != nil {
		confOverride(bxmConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return bxm.New(ctx, bxmConf) }); err != nil {
		t.Fatalf("failed to register BitMED protocol: %v", err)
	}
