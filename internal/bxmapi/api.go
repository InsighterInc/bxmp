...
// PublicBitmedAPI provides an API to access BitMED related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBitmedAPI struct {
	b Backend
}

// NewPublicBitmedAPI creates a new BitMED protocol API.
func NewPublicBitmedAPI(b Backend) *PublicBitmedAPI {
	return &PublicBitmedAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicBitmedAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current BitMED protocol version this node supports
func (s *PublicBitmedAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicBitmedAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Bitmed Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Bitmed Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an BitMED ECDSA signature for:
// keccack256("\x19Bitmed Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/InsighterInc/bxmp/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with bxm_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Bitmed Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/InsighterInc/bxmp/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid BitMED signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the BitMED blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new BitMED blockchain API.
...
func (s *PublicBlockChainAPI) doCall(ctx context.Context, args CallArgs, blockNr rpc.BlockNumber, vmCfg vm.Config) ([]byte, *big.Int, bool, error) {
	...
	if gasPrice.Sign() == 0 && !s.b.ChainConfig().IsBitmed {
...
func (s *PublicTransactionPoolAPI) sign(addr common.Address, tx *types.Transaction) (*types.Transaction, error) {
	...
	return wallet.SignTx(account, tx, chainID, isBitmed)
}
...
func (s *PublicTransactionPoolAPI) SendTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	signed, err := wallet.SignTx(account, tx, chainID, isBitmed)
	if err != nil {
		return common.Hash{}, err
	}
	return submitTransaction(ctx, s.b, signed, isPrivate)
...
//
// https://github.com/bitmed/wiki/wiki/JSON-RPC#bxm_sign
...
// PublicDebugAPI is the collection of BitMED APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of BitMED APIs exposed over the private
// debugging endpoint.
...
// Version returns the current bitmed protocol version.
...
// date when account management is handled outside BitMED.
...
// GetBitmedPayload returns the contents of a private transaction
func (s *PublicBlockChainAPI) GetBitmedPayload(digestHex string) (string, error) {
	if private.P == nil {
	...
	if len(b) != 64 {
		return "", fmt.Errorf("Expected a BitMED digest of length 64, but got %d", len(b))
	}
