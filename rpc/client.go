...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout bxm_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for BxmSubscribe requests
}
...
// BxmSubscribe registers a subscripion under the "bxm" namespace.
func (c *Client) BxmSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "bxm", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. BxmSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through BxmSubscribe.
