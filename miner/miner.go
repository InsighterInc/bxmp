...
// Package miner implements BitMED block creation and mining.
package miner
...
func New(bxm Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		bxm:      bxm,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, bxm, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(bxm.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setBxmbase(coinbase)
...
func (self *Miner) SetExtra(extra []byte) error {
	if uint64(len(extra)) > params.GetMaximumExtraDataSize(self.worker.chain.Config().IsBitmed) {
		return fmt.Errorf("Extra exceeds max length. %d > %v", len(extra), params.GetMaximumExtraDataSize(self.worker.chain.Config().IsBitmed))
	}
...
func (self *Miner) SetBxmbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setBxmbase(addr)
}
