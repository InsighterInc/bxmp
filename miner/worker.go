...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, bxm Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		engine:         engine,
		bxm:            bxm,
		...
		chain:          bxm.BlockChain(),
		proc:           bxm.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(bxm.BlockChain(), miningLogAtDepth),
	}

	if _, ok := engine.(consensus.Istanbul); ok || !config.IsBitmed {
		// Subscribe TxPreEvent for tx pool
		worker.txSub = bxm.TxPool().SubscribeTxPreEvent(worker.txCh)
		// Subscribe events for blockchain
		worker.chainHeadSub = bxm.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
		worker.chainSideSub = bxm.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
		go worker.update()
...
func (self *worker) setBxmbase(addr common.Address) {
	self.mu.Lock()
...
func (self *worker) commitNewWork() {
	...
	pending, err := self.bxm.TxPool().Pending()
	if err != nil {
