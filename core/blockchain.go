...
// Package core implements the BitMED consensus protocol.
package core
...
// available in the database. It initialises the default BitMED Validator and
// Processor.
func NewBlockChain(chainDb bxmdb.Database, config *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= bxmdb.IdealBatchSize {
	...
	for i, block := range chain {
		...
		if err != nil {
			...
			if err == consensus.ErrFutureBlock {
				...
				if block.Time().Cmp(max) > 0 && !bc.config.IsBitmed {
