...
var (
	...
	configPrefix   = []byte("bitmed-config-") // config prefix for the db
...
func WriteCanonicalHash(db bxmdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db bxmdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db bxmdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db bxmdb.Putter, hash common.Hash) error {
...
func WriteHeader(db bxmdb.Putter, header *types.Header) error {
...
func WriteBody(db bxmdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db bxmdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db bxmdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db bxmdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db bxmdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db bxmdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db bxmdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db bxmdb.Database) bxmdb.Database {
	return bxmdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db bxmdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db bxmdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db bxmdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
...
func GetPrivateStateRoot(db bxmdb.Database, blockRoot common.Hash) common.Hash {
	root, _ := db.Get(append(privateRootPrefix, blockRoot[:]...))
	return common.BytesToHash(root)
}

func WritePrivateStateRoot(db bxmdb.Database, blockRoot, root common.Hash) error {
...
func WritePrivateBlockBloom(db bxmdb.Database, number uint64, receipts types.Receipts) error {
...
func GetPrivateBlockBloom(db bxmdb.Database, number uint64) (bloom types.Bloom) {
