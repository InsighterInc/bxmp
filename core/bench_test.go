...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db bxmdb.Database
	if !disk {
		db, _ = bxmdb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "bxm-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = bxmdb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
...
// into a database.
func makeChainForBench(db bxmdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "bxm-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		db, err := bxmdb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
