...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	isBitmed := pool.chainconfig.IsBitmed

	if isBitmed && tx.GasPrice().Cmp(common.Big0) != 0 {
	...
	if !isBitmed && !local && pool.gasPrice.Cmp(tx.GasPrice()) > 0 {
	...
	if !isBitmed && tx.Gas().Cmp(intrGas) < 0 {
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (bool, error) {
	...
	if uint64(len(pool.all)) >= pool.config.GlobalSlots+pool.config.GlobalQueue {
		...
		if !pool.chainconfig.IsBitmed && pool.priced.Underpriced(tx, pool.locals) {
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) {
	...
	if isBitmed && pool.pendingState == nil {
		pool.reset(nil, nil)
	}
