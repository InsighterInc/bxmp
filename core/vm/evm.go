...
// note: BitMED, States, and Value Transfer
//
// In BitMED there is a tricky issue in one specific case when there is call from private state to public state:
...
//
// The solution is to skip this transfer of 0 value under BitMED
...
// EVM is the BitMED Virtual Machine base object and provides
...
type EVM struct {
	...
	// evm.
	vmConfig Config
	// global (to this context) bitmed virtual machine
	...
	// be simplified). This is set by BitMED when it's inside a Private State -> Public State read.
	quorumReadOnly bool
	readOnlyDepth  uint
}
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if evm.ChainConfig().IsBitmed {
		// skip transfer if value /= 0 (see note: BitMED, States, and Value Transfer)
		if value.Sign() != 0 {
