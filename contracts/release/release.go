...
type Config struct {
	Oracle common.Address // BitMED address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the BitMED service dependency to access the blockchain
	var apiBackend bxmapi.Backend
	var bitmed *bxm.BitMED
	if err := ctx.Service(&bitmed); err == nil {
		apiBackend = bitmed.ApiBackend
	} else {
		var bitmed *les.LightBitmed
		if err := ctx.Service(&bitmed); err == nil {
			apiBackend = bitmed.ApiBackend
		} else {
			return nil, err
	...
	contract, err := NewReleaseOracle(config.Oracle, bxm.NewContractBackend(apiBackend))
	if err != nil {
		return nil, err
	}
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/InsighterInc/bxmp/releases for new releases")
