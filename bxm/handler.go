...
import (
	...
	"github.com/InsighterInc/bxmp/bxm/fetcher"
...
// NewProtocolManager returns a new bitmed sub protocol manager. The BitMED sub protocol manages peers capable
// with the bitmed network.
...
// Figure out whether to allow fast sync or not
	...
	for i, version := range protocol.Versions {
		...
		if mode == downloader.FastSync && version < consensus.Bxm63 {
			continue
		}
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing BitMED peer", "peer", id)

	// Unregister the peer from the downloader and BitMED peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping BitMED protocol")
	...
	log.Info("BitMED protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an bxm peer. When
// this function terminates, the peer is disconnected.
func (pm *ProtocolManager) handle(p *peer) error {
	if pm.peers.Len() >= pm.maxPeers {
		return p2p.DiscTooManyPeers
	}
	p.Log().Debug("BitMED peer connected", "name", p.Name())

	// Execute the BitMED handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("BitMED handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("BitMED peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("BitMED message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= consensus.Bxm63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= consensus.Bxm63 && msg.Code == NodeDataMsg:
	...
	case p.version >= consensus.Bxm63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= consensus.Bxm63 && msg.Code == ReceiptsMsg:
...
// BxmNodeInfo represents a short summary of the BitMED sub-protocol metadata known
// about the host peer.
