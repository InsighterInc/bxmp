...
func TestProtocolCompatibility(t *testing.T) {
	...
	backup := consensus.BxmProtocol.Versions
	defer func() { consensus.BxmProtocol.Versions = backup }()

	// Try all available compatibility configs and check for errors
	for i, tt := range tests {
		consensus.BxmProtocol.Versions = []uint{tt.version}
...
func testGetNodeData(t *testing.T, protocol int) {
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*bxmdb.MemDatabase).Keys() {
	...
	statedb, _ := bxmdb.NewMemDatabase()
...
// Tests that post bxm protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", consensus.Bxm63, pm, true)
	defer peer.close()
