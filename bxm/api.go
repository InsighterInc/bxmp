...
import (
	...
	"github.com/InsighterInc/bxmp/trie"
)

const defaultTraceTimeout = 5 * time.Second

// PublicBitmedAPI provides an API to access BitMED full node-related
// information.
...
// PrivateAdminAPI is the collection of BitMED full node-related APIs
...
// admin methods of the BitMED service.
func NewPrivateAdminAPI(bxm *BitMED) *PrivateAdminAPI {
	return &PrivateAdminAPI{bxm: bxm}
}
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.bxm.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.bxm.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.bxm.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of BitMED full node APIs exposed
...
// related public debug methods of the BitMED service.
func NewPublicDebugAPI(bxm *BitMED) *PublicDebugAPI {
	return &PublicDebugAPI{bxm: bxm}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber, typ string) (state.Dump, error) {
	...
	if blockNr == rpc.PendingBlockNumber {
		...
		_, publicState, privateState = api.bxm.miner.Pending()
	} else {
		var block *types.Block
		if blockNr == rpc.LatestBlockNumber {
			block = api.bxm.blockchain.CurrentBlock()
		} else {
			block = api.bxm.blockchain.GetBlockByNumber(uint64(blockNr))
		}
		if block == nil {
			return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
		}
		publicState, privateState, err = api.bxm.BlockChain().StateAt(block.Root())
		if err != nil {
...
// PrivateDebugAPI is the collection of BitMED full node APIs exposed over
...
// private debug methods of the BitMED service.
func NewPrivateDebugAPI(config *params.ChainConfig, bxm *BitMED) *PrivateDebugAPI {
	return &PrivateDebugAPI{config: config, bxm: bxm}
}
...
func (api *PrivateDebugAPI) TraceBlock(blockRlp []byte, config *vm.LogConfig) BlockTraceResult {
	...
	return BlockTraceResult{
		Validated:  validated,
		StructLogs: bxmapi.FormatLogs(logs),
...
func (api *PrivateDebugAPI) TraceBlockByNumber(blockNr rpc.BlockNumber, config *vm.LogConfig) BlockTraceResult {
	...
	case rpc.PendingBlockNumber:
		...
		block = api.bxm.miner.PendingBlock()
...
func (api *PrivateDebugAPI) TraceBlockByHash(hash common.Hash, config *vm.LogConfig) BlockTraceResult {
	// Fetch the block that we aim to reprocess
	block := api.bxm.BlockChain().GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(block *types.Block, logConfig *vm.LogConfig) (bool, []vm.StructLog, error) {
	...
	if err := api.bxm.engine.VerifyHeader(blockchain, block.Header(), true); err != nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, txHash common.Hash, config *TraceArgs) (interface{}, error) {
	...
	if config != nil && config.Tracer != nil {
		...
		var err error
		if tracer, err = bxmapi.NewJavascriptTracer(*config.Tracer); err != nil {
			return nil, err
		}
		...
		go func() {
			<-deadlineCtx.Done()
			tracer.(*bxmapi.JavascriptTracer).Stop(&timeoutError{})
		}()
		defer cancel()
	} else if config == nil {
	...
	tx, blockHash, _, txIndex := core.GetTransaction(api.bxm.ChainDb(), txHash)
	if tx == nil {
	...
	case *vm.StructLogger:
		return &bxmapi.ExecutionResult{
			Gas:         gas,
			Failed:      failed,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  bxmapi.FormatLogs(tracer.StructLogs()),
		}, nil
	case *bxmapi.JavascriptTracer:
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int) (core.Message, vm.Context, *state.StateDB, *state.StateDB, error) {
	...
	block := api.bxm.BlockChain().GetBlockByHash(blockHash)
	if block == nil {
		return nil, vm.Context{}, nil, nil, fmt.Errorf("block %x not found", blockHash)
	}
	parent := api.bxm.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, vm.Context{}, nil, nil, fmt.Errorf("block parent %x not found", block.ParentHash())
	}
	statedb, privateStateDb, err := api.bxm.BlockChain().StateAt(parent.Root())
	if err != nil {
	...
	for idx, tx := range txs {
		...
		context := core.NewEVMContext(msg, block.Header(), api.bxm.BlockChain(), nil)
		if idx == txIndex {
...
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	db := core.PreimageTable(api.bxm.ChainDb())
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]core.BadBlockArgs, error) {
	return api.bxm.BlockChain().BadBlocks()
}
