...
import (
	...
	bitmed "github.com/InsighterInc/bxmp"
...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [bxm/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [bxm/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [bxm/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [bxm/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [bxm/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [bxm/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [bxm/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [bxm/63] Channel receiving inbound node state data
...
func New(mode SyncMode, stateDb bxmdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
// these are zero.
func (d *Downloader) Progress() bitmed.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "bxm", p.version, "head", hash, "td", td, "mode", d.mode)
