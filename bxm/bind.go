...
// ContractBackend implements bind.ContractBackend with direct calls to BitMED
// internals to support operating on contracts within subprotocols like bxm and
// swarm.
//
// Internally this backend uses the already exposed API endpoints of the BitMED
// object. These should be rewritten to internal Go method calls when the Go API
// is refactored to support a clean library use.
type ContractBackend struct {
	eapi  *bxmapi.PublicBitmedAPI        // Wrapper around the BitMED object to access metadata
	bcapi *bxmapi.PublicBlockChainAPI      // Wrapper around the blockchain to access chain data
	txapi *bxmapi.PublicTransactionPoolAPI // Wrapper around the transaction pool to access transaction data
}
...
func NewContractBackend(apiBackend bxmapi.Backend) *ContractBackend {
	return &ContractBackend{
		eapi:  bxmapi.NewPublicBitmedAPI(apiBackend),
		bcapi: bxmapi.NewPublicBlockChainAPI(apiBackend),
		txapi: bxmapi.NewPublicTransactionPoolAPI(apiBackend, new(bxmapi.AddrLocker)),
	}
}
...
// ContractCall implements bind.ContractCaller executing an BitMED contract
...
func toCallArgs(msg bitmed.CallMsg) bxmapi.CallArgs {
	args := bxmapi.CallArgs{
		To:   msg.To,
		From: msg.From,
		Data: msg.Data,
	}
