...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (bxmdb.Database, error) {
	if ctx.config.DataDir == "" {
		return bxmdb.NewMemDatabase()
	}
	db, err := bxmdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
