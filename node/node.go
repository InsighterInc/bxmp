...
func (n *Node) OpenDatabase(name string, cache, handles int) (bxmdb.Database, error) {
	if n.config.DataDir == "" {
		return bxmdb.NewMemDatabase()
	}
	return bxmdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
