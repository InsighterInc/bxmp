...
import (
	...
	"github.com/InsighterInc/bxmp/les/flowcontrol"
...
func NewLesServer(bxm *bxm.BitMED, config *bxm.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(bxm.BlockChain().Config(), false, config.NetworkId, bxm.EventMux(), bxm.Engine(), newPeerSet(), bxm.BlockChain(), bxm.TxPool(), bxm.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	pm.blockLoop()
	...
	srv := &LesServer{
		protocolManager: pm,
		quitSync:        quitSync,
		lesTopic:        lesTopic(bxm.BlockChain().Genesis().Hash()),
	}
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(bxm.ChainDb())
	return srv, nil
}
...
func newCostStats(db bxmdb.Database) *requestCostStats {
...
func getChtRoot(db bxmdb.Database, num uint64) common.Hash {
...
func storeChtRoot(db bxmdb.Database, num uint64, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], num)
	db.Put(append(chtPrefix, encNumber[:]...), root[:])
}

func makeCht(db bxmdb.Database) bool {
