...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.bitmed", new NodeConfig());
			node.start();
			...
			BitmedClient ec = node.getBitmedClient();
	...
	// signatures too. Regression test for BXMP issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/bitmed/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.bitmed", "github.com/InsighterInc/bxmp/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "bitmed", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
