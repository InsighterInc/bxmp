...
func (w *wizard) networkStats(tips bool) {
	...
	for server, pubkey := range w.conf.Servers {
		...
		logger.Debug("Checking for bxmstats availability")
		if infos, err := checkBxmstats(client, w.network); err != nil {
			if err != ErrServiceUnknown {
				services["bxmstats"] = err.Error()
			}
		} else {
			services["bxmstats"] = infos.String()
			protips.bxmstats = infos.config
		}
	...
	if protips.bxmstats != "" {
		w.conf.bxmstats = protips.bxmstats
	}
...
func (p *protips) print(network string) {
	...
	// If an bxmstats server is available, add the bxmstats flag
	statsflag := ""
	if p.bxmstats != "" {
		if strings.Contains(p.bxmstats, " ") {
			statsflag = fmt.Sprintf(` --bxmstats="yournode:%s"`, p.bxmstats)
		} else {
			statsflag = fmt.Sprintf(` --bxmstats=yournode:%s`, p.bxmstats)
		}
	}
