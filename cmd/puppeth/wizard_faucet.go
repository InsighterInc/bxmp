...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many BXMs to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.bxmstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.bxmstats = w.readString() + ":" + w.conf.bxmstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.bxmstats)
		infos.node.bxmstats = w.readDefaultString(infos.node.bxmstats) + ":" + w.conf.bxmstats
	}
