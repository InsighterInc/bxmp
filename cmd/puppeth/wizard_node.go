...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.bxmstats == "" {
		log.Error("No bxmstats server configured")
		return
	}
	...
	if infos.bxmstats == "" {
		...
		infos.bxmstats = w.readString() + ":" + w.conf.bxmstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.bxmstats)
		infos.bxmstats = w.readDefaultString(infos.bxmstats) + ":" + w.conf.bxmstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.genesis.Config.Ethash != nil {
			// Ethash based miners only need an bxmbase to mine against
			fmt.Println()
			if infos.bxmbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.bxmbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.bxmbase)
				infos.bxmbase = w.readDefaultAddress(common.HexToAddress(infos.bxmbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy BitMED node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
