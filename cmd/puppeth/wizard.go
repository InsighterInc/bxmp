...
type config struct {
	...
	bxmstats  string        // Bxmstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // BitMED services known to be running on servers
...
// converts it to an BitMED address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by bxmstats, which compares IPs textually, not by value.
