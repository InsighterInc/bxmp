...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"bxmstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "bxmstats":
				if infos, err := checkBxmstats(client, w.network); err == nil {
					port = infos.port
				}
			case "faucet":
	...
	// If we have bxmstats running, ask whether to make the secret public or not
	var bxmstats bool
	if w.conf.bxmstats != "" {
		fmt.Println()
		fmt.Println("Include bxmstats secret on dashboard (y/n)? (default = yes)")
		bxmstats = w.readDefaultString("y") == "y"
	}
	// Try to deploy the dashboard container on the host
	if out, err := deployDashboard(client, w.network, infos.port, infos.host, listing, &w.conf, bxmstats); err != nil {
