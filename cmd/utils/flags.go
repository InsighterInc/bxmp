...
// Package utils contains internal helper functions for BXMP commands.
package utils
...
import (
	...
	"github.com/InsighterInc/bxmp/accounts/keystore"
	...
	"github.com/InsighterInc/bxmp/bxmstats"
	"github.com/InsighterInc/bxmp/les"
	...
	"github.com/InsighterInc/bxmp/p2p/nat"
	"github.com/InsighterInc/bxmp/p2p/netutil"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		...
		Value: bxm.DefaultConfig.NetworkId,
	}
	...
	defaultSyncMode = bxm.DefaultConfig.SyncMode
	...
	EthashCachesInMemoryFlag = cli.IntFlag{
		...
		Value: bxm.DefaultConfig.EthashCachesInMem,
	}
	EthashCachesOnDiskFlag = cli.IntFlag{
		Name:  "ethash.cachesondisk",
		Usage: "Number of recent ethash caches to keep on disk (16MB each)",
		Value: bxm.DefaultConfig.EthashCachesOnDisk,
	}
	EthashDatasetDirFlag = DirectoryFlag{
		Name:  "ethash.dagdir",
		Usage: "Directory to store the ethash mining DAGs (default = inside home folder)",
		Value: DirectoryString{bxm.DefaultConfig.EthashDatasetDir},
	}
	EthashDatasetsInMemoryFlag = cli.IntFlag{
		Name:  "ethash.dagsinmem",
		Usage: "Number of recent ethash mining DAGs to keep in memory (1+GB each)",
		Value: bxm.DefaultConfig.EthashDatasetsInMem,
	}
	EthashDatasetsOnDiskFlag = cli.IntFlag{
		Name:  "ethash.dagsondisk",
		Usage: "Number of recent ethash mining DAGs to keep on disk (1+GB each)",
		Value: bxm.DefaultConfig.EthashDatasetsOnDisk,
	}
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: bxm.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolPriceBumpFlag = cli.Uint64Flag{
		Name:  "txpool.pricebump",
		Usage: "Price bump percentage to replace an already existing transaction",
		Value: bxm.DefaultConfig.TxPool.PriceBump,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: bxm.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: bxm.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: bxm.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: bxm.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: bxm.DefaultConfig.TxPool.Lifetime,
	}
	...
	GasPriceFlag = BigFlag{
		...
		Value: bxm.DefaultConfig.GasPrice,
	}
	...
	BxmStatsURLFlag = cli.StringFlag{
		Name:  "bxmstats",
		Usage: "Reporting URL of a bxmstats service (nodename:secret@host:port)",
	}
	...
	GpoBlocksFlag = cli.IntFlag{
		...
		Value: bxm.DefaultConfig.GPO.Blocks,
	}
	GpoPercentileFlag = cli.IntFlag{
		Name:  "gpopercentile",
		Usage: "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value: bxm.DefaultConfig.GPO.Percentile,
	}
	...
	RaftModeFlag = cli.BoolFlag{
		Name:  "raft",
		Usage: "If enabled, uses Raft instead of BitMED Chain for consensus",
	}
	...
	IstanbulRequestTimeoutFlag = cli.Uint64Flag{
		...
		Value: bxm.DefaultConfig.Istanbul.RequestTimeout,
	}
	IstanbulBlockPeriodFlag = cli.Uint64Flag{
		Name:  "istanbul.blockperiod",
		Usage: "Default minimum difference between two consecutive block's timestamps in seconds",
		Value: bxm.DefaultConfig.Istanbul.BlockPeriod,
	}
	IstanbulBlockPauseTimeFlag = cli.Uint64Flag{
		...
		Value: bxm.DefaultConfig.Istanbul.BlockPauseTime,
	}
)
...
// setBxmbase retrieves the bxmbase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func setBxmbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *bxm.Config) {
	if ctx.GlobalIsSet(BxmbaseFlag.Name) {
		account, err := MakeAddress(ks, ctx.GlobalString(BxmbaseFlag.Name))
		if err != nil {
			Fatalf("Option %q: %v", BxmbaseFlag.Name, err)
		}
		cfg.Bxmbase = account.Address
		return
	}
	accounts := ks.Accounts()
	if (cfg.Bxmbase == common.Address{}) {
		if len(accounts) > 0 {
			cfg.Bxmbase = accounts[0].Address
		} else {
			log.Warn("No bxmbase set and no accounts found as default")
		}
	}
}
...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(DevModeFlag.Name):
		cfg.DataDir = filepath.Join(os.TempDir(), "bitmed_dev_mode")
	case ctx.GlobalBool(TestnetFlag.Name):
...
func setEthash(ctx *cli.Context, cfg *bxm.Config) {
...
func setIstanbul(ctx *cli.Context, cfg *bxm.Config) {
...
// SetEthConfig applies bxm-related command line flags to the config.
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *bxm.Config) {
	// Avoid conflicting network flags
	checkExclusive(ctx, DevModeFlag, TestnetFlag, RinkebyFlag, OttomanFlag)
	checkExclusive(ctx, FastSyncFlag, LightModeFlag, SyncModeFlag)

	ks := stack.AccountManager().Backends(keystore.KeyStoreType)[0].(*keystore.KeyStore)
	setBxmbase(ctx, ks, cfg)
	setGPO(ctx, &cfg.GPO)
...
// RegisterEthService adds an BitMED client to the stack.
func RegisterEthService(stack *node.Node, cfg *bxm.Config) <-chan *bxm.BitMED {
	nodeChan := make(chan *bxm.BitMED, 1)
	var err error
	...
	if err != nil {
		Fatalf("Failed to register the BitMED service: %v", err)
	}
...
// RegisterBxmStatsService configures the BitMED Stats daemon and adds it to
// th egiven node.
func RegisterBxmStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both bxm and les services
		var bxmServ *bxm.BitMED
		ctx.Service(&bxmServ)

		var lesServ *les.LightBitmed
		ctx.Service(&lesServ)

		return bxmstats.New(url, bxmServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the BitMED Stats service: %v", err)
	}
}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) bxmdb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb bxmdb.Database) {
	var err error
	...
	if config.Clique != nil {
		...
		if !ctx.GlobalBool(FakePoWFlag.Name) {
			engine = ethash.New(
				stack.ResolvePath(bxm.DefaultConfig.EthashCacheDir), bxm.DefaultConfig.EthashCachesInMem, bxm.DefaultConfig.EthashCachesOnDisk,
				stack.ResolvePath(bxm.DefaultConfig.EthashDatasetDir), bxm.DefaultConfig.EthashDatasetsInMem, bxm.DefaultConfig.EthashDatasetsOnDisk,
			)
		}
	}
