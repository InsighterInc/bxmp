...
Use "bitmed dump 0" to dump the genesis block.`,
	}
)

// In the regular Genesis / ChainConfig struct, due to the way go deserializes
// json, IsBitmed defaults to false (when not specified). Here we specify it as
// a pointer so we can make the distinction and default unspecified to true.
func getIsBitmed(file io.Reader) bool {
	altGenesis := new(struct {
		Config *struct {
			IsBitmed *bool `json:"isBitmed"`
		} `json:"config"`
	})
	...
	return altGenesis.Config.IsBitmed == nil || *altGenesis.Config.IsBitmed
}
...
func initGenesis(ctx *cli.Context) error {
	...
	file.Seek(0, 0)
	genesis.Config.IsBitmed = getIsBitmed(file)
...
func copyDb(ctx *cli.Context) error {
	...
	db, err := bxmdb.NewLDBDatabase(ctx.Args().First(), ctx.GlobalInt(utils.CacheFlag.Name), 256)
	if err != nil {
		return err
	}
	...
	start = time.Now()
	fmt.Println("Compacting entire database...")
	if err = chainDb.(*bxmdb.LDBDatabase).LDB().CompactRange(util.Range{}); err != nil {
		utils.Fatalf("Compaction failed: %v", err)
	}
	fmt.Printf("Compaction done in %v.\n\n", time.Since(start))
