...
// faucet is a BXM faucet backed by a light client.
package main
...
var (
	...
	bxmPortFlag = flag.Int("bxmport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the BitMED protocol")
	statsFlag   = flag.String("bxmstats", "", "Bxmstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of BXMs to pay out per user request")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for BitMED and the faucet")
)

var (
	bxms = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s BXMs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *bxmPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // BitMED address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an BitMED light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // BitMED protocol stack
	client *bxmclient.Client   // Client connection to the BitMED chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the BitMED light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := bxm.DefaultConfig
	...
	// Assemble the bxmstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightBitmed
			ctx.Service(&serv)
			return bxmstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := bxmclient.NewClient(api)
...
// close terminates the BitMED connection and tears down the faucet.
...
// apiHandler handles requests for BXM grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	websocket.JSON.Send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, bxms),
		"funded":   nonce,
	...
	for {
		...
		// Iterate over all the files and look for BitMED addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No BitMED address found to fund"})
			continue
		}
		...
		if timeout = f.timeouts[gist.Owner.Login]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), bxms)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance, _ := f.client.BalanceAt(context.Background(), f.account.Address, nil)
			balance = new(big.Int).Div(balance, bxms)
