...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the BitMED
...
type wallet struct {
	...
	deriveChain    bitmed.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived BitMED account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain bitmed.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
func (w *wallet) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int, isBitmed bool) (*types.Transaction, error) {
	w.stateLock.RLock() // Comms have own mutex, this is for the state fields
	defer w.stateLock.RUnlock()

	if isBitmed {
		return nil, errors.New("Signing BitMED transactions with a USB wallet not yet supported")
	}
