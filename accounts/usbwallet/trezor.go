...
import (
	...
	"github.com/InsighterInc/bxmp/accounts/usbwallet/internal/trezor"
...
// or whether the BitMED app was not started on it.
...
// BitMED address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.BitmedAddress)
	if _, err := w.trezorExchange(&trezor.BitmedGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.BitmedTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.BitmedTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
