...
func (w *keystoreWallet) SelfDerive(base accounts.DerivationPath, chain bitmed.ChainStateReader) {}
...
func (w *keystoreWallet) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int, isBitmed bool) (*types.Transaction, error) {
	// Make sure the requested account is contained within
	if account.Address != w.account.Address {
		return nil, accounts.ErrUnknownAccount
	}
	...
	// Account seems valid, request the keystore to sign
	return w.keystore.SignTx(account, tx, chainID, isBitmed)
}
